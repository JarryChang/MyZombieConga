...
- (void)boundsCheckPlayer
{
  ...
  // 2
  CGPoint bottomLeft = [_bgLayer convertPoint:CGPointZero fromNode:self];//CGPointZero;
  CGPoint topRight = [_bgLayer convertPoint:CGPointMake(self.size.width, self.size.height) fromNode:self];//CGPointMake(self.size.width,self.size.height);
...
- (void)spawnCat
{
  ...
  cat.position = [_bgLayer convertPoint:catScenePos toNode:self];
  cat.xScale = 0;
  cat.yScale = 0;
//  [self addChild:cat];
